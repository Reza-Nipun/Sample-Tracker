	...
	if (isset($_POST['search']))
 		{
		  ...
		  $date=$_POST['date'];	
		  $date1=$_POST['date1'];	
		  $select_logicm=$_POST['select_logic'];
		  
		  	$select_logic_temp=explode("~", $select_logicm);
			$select_logic = $select_logic_temp[0];
			$select_logic_view = $select_logic_temp[1];
...
<body>
   ...
   <div class="span12">
                                <div style="float:left"><h2>Update STO No</h2></div>
								<div style="color:#39F; padding-top:5px; padding-left:165px; font-weight:bold">(You can Update STO only before Dying has create their Job Reference against your STO !!!)</div>
								<?php
...
<form action="" method="post" >    
               ...
               <table width="989" border="1" align="left" cellpadding="0" cellspacing="0" class="bottomBorder" id="gradient-style">
  <tr align="center" style="font-weight:bold; font-size:11px">
  
    <td>Search By </td>
    <td>
      <select name="select_logic" id="select_logic">
      <?php if($select_logic != '') echo '<option selected value="'.$select_logicm.'">'.$select_logic_view.'</option>'; 
	  
	  
	   ?>
      
        <option value="a.mm_date~STO Create Date">STO Create Date</option>
        <option value="a.dpd_date~Fabric Booking Create Date">Fabric Booking Create Date</option>
        <option value="b.fab_receive_planned_date~Fabric Receive Planned Date">Fabric Receive Planned Date</option>
        <option value="b.sd_agreed_sample_delivery_date~Agreed Sample Delivery Date">Agreed Sample Delivery Date</option>
      </select></td>
...
<td>Date from</td>
    <td>
    <input name="date" class="rounded" value="<?php echo $date; ?>" type="text" id="date" placeholder="dd-mm-yyyy" tabindex="2" onclick="showCalendarControl(this);" /></td>
    <td>Date To</td>
    <td><input name="date1" class="rounded" value="<?php echo $date1; ?>" type="text" id="date1" placeholder="dd-mm-yyyy" tabindex="2" onclick="showCalendarControl(this);" /></td>
    <!--<input type="text" name="sd_concern_sd_name" id="sd_concern_sd_name">-->
    
    <td>STO No</td>
    <td><input name="sto_no" class="rounded" value="<?php echo $sto_no; ?>" type="text" id="sto_no" tabindex="3" /></td>
...
<?php if($search == 1) { ?>
        ...
        //	$SQL="select *,a.sl as sll,a.color_code as c_code from tb_fabric_booking a,tb_track_info b WHERE b.sd_concern_mm_name='$user_name' AND a.sample_style=b.style AND b.color=a.color";    //table name
            ...
            $sql="select a.*,a.sl as sll,a.sample_style,a.color_code as c_code,b.sd_concern_mm_name,b.customer,b.fab_receive_planned_date,b.sd_sample_type_name, b.sd_concern_dpd_name,b.sd_agreed_sample_delivery_date from tb_fabric_booking a,tb_track_info b WHERE a.track_info_sl=b.sl AND a.cancel_status = 0 AND referrence_id = 0";    //table name
       ...
       if($date!='' || $date1 != '')
       {
			...
			if($date!='' && $date1 == '')
			  {
			  $sql= $sql . " AND $select_logic = '$date'" ; 
			  }
			if($date=='' && $date1 != '')
			  {
			  $sql= $sql . " AND $select_logic = '$date1'" ; 
			  }
		}
			...
			if($sto_no != '')
			 {
			$sql= $sql . " AND a.sto_po_no LIKE '%$sto_no%'";	 
			 }
			 else
			 {
        ...
        if($sys_date!='')
        {
             $sql= $sql . " AND a.sto_po_no != '' AND a.cancel_status = 0 AND referrence_id = 0 group by a.sl order by sll ASC" ;
        }*/
        ...
        <td><?php echo $row['sd_concern_dpd_name'] ; ?></td>
             ...
             <span id="sto_<?php echo $row['sll']; ?>"><?php echo $row['sto_po_no'];?></span>
                  
                                <input type="text" class="editbox" onClick="this.select()" onFocus="this.select()" id="first_input_<?php echo $row['sll']; ?>" value="<?php echo $row['sto_po_no']; ?>" size="20" maxlength="15" /></td>
