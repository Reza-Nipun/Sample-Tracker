...
$sd_clarification_arng_cnfrm_from_buyer_date_post=mysql_real_escape_string($_POST['sd_clarification_arng_cnfrm_from_buyer_date']);
$sd_agreed_sample_delivery_date_post=mysql_real_escape_string($_POST['sd_agreed_sample_delivery_date']);

if($sd_clarification_arng_confrm_from_buyer_date!=NULL && $sd_agreed_sample_delivery_date!=NULL)
{
	die('ERROR!!!.Data already Updated.');
}

if(strlen($sd_clarification_arng_cnfrm_from_buyer_date_post)>0 && strlen($sd_agreed_sample_delivery_date_post)>0)
{
	
$SQL="UPDATE `tb_track_info` SET sd_clarification_arng_confrm_from_buyer_date = '$sd_clarification_arng_cnfrm_from_buyer_date_post' ,`sd_agreed_sample_delivery_date` = '$sd_agreed_sample_delivery_date_post' WHERE `tb_track_info`.`sl` ='$sl' AND brand_style='$update_brand_style'" ;
$obj->sql($SQL);	
