...
<input type="button" value="Delete Row" onClick="deleteRow('gradientstyle')" /><br>
    ...
    <table class="bottomBorder" id="gradientstyle" width="80%" border="1" cellspacing="0"  cellpadding="0">
	  ...
	  if($user_rule=='1')
	  {
	   $SQL="select style from tb_track_info WHERE sd_concern_sd_name='$user_name' group by style order by style ASC";
	  }
 ...
 if($user_rule=='3')
	  {
$SQL="select color from tb_track_info WHERE sd_concern_dpd_name='$user_name' group by color order by color ASC";
	  }
	  if($user_rule=='1')
	  {
$SQL="select color from tb_track_info WHERE sd_concern_sd_name='$user_name' group by color order by color ASC";
