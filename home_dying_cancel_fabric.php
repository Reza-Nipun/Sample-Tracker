...
<th>&nbsp;</th>
  <th><strong>SL</strong></th>
    ...
    <th>Cancel Cmnts</th>
		...
		<tr>
		  <td><input type="hidden" name="" size="2" value="" class="search_init" /></td>
			...
			<td><input type="text" name="Cancel Date" size="4" value="Cancel Date" class="search_init" /></td>
	<td><input type="text" name="Fab Rec. Planned Date" size="4" value="Fab Rec. Planned Date" class="search_init" /></td>
            ...
            <td><input type="text" name="Cancel Cmnts" size="4" value="Cancel Cmnts" class="search_init" /></td>
            <td><input type="text" name="Cancel Reason" size="4" value="Cancel Reason" class="search_init" /></td>
	  </tr>
...
//	$results  = $obj->sql($SQL);
	...
	$sql="select a.*,b.fab_receive_planned_date,b.customer,b.sd_sample_type_name,b.sd_agreed_sample_delivery_date, b.sd_concern_dpd_name,a.sl as sll,a.color_code as c_code from tb_fabric_booking a,tb_track_info b WHERE a.track_info_sl=b.sl ";    //table name
	...
	// $sql= $sql . " AND a.sto_po_no LIKE '55%' AND a.del_status!='Complete' group by a.sl order by sll ASC " ;
	 $sql= $sql . " AND a.cancel_status <> 0 group by a.sl order by LENGTH(mm_date), mm_date ASC" ;
$result = $obj->sql($sql);
    ...
    <tr id="<?php echo $row['sll'] ; ?>" class="edit_tr" style="color:#F00">
		  ...
		  <td><label><input name="checkbox[]" type="checkbox" id="checkbox[]" value="<?php echo $row['sll']; ?>"></label></td>
    ...
    <td bgcolor="#FFFFCC" style="color:#000"><?php echo $row['sto_po_no'] ;  ?></td>
    <td bgcolor="#FFFFCC" style="color:#000"><?php echo $row['mm_date'] ;  ?></td>
...
<input name="submit" type="submit" class="btn btn-success" id="submit" value="Print Preview »" />
      &nbsp;
      <input name="input" type="reset" class="btn btn-danger" value="Reset" />
</form> 
