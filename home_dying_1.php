<?php
	...
	$user_name=$row['user_name'];
		...
		$search = 0; // set default value as 0 to avoid loading all data initially.
...
<td bgcolor="#93C9FF" <?php if($cancel_status != 0) { ?> style="color:#F00"<?php } ?>><?php echo $row['fab_wash'] ;  ?></td>
...
<td<?php if($revise_status != 0) { ?> bgcolor="#88E4EC" <?php } if($cancel_status != 0) { ?> style="color:#F00"<?php } ?>><?php echo $row['remark_dpd'] ;  ?></td>
 ...
 <td><?php echo $row['supplier'] ;  ?></td>
    ...
    <td bgcolor="#FF3300" style="font-size:14px"><?php echo trim($row['dpd_req_qty']-$row['delv_qty']) ; ?></td>
