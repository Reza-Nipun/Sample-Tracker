...
$dpd_meeting_sd_mm_dpd_sample_date_post=mysql_real_escape_string($_POST['dpd_meeting_sd_mm_dpd_sample_date']);

if($dpd_meeting_sd_mm_dpd_sample_date!=NULL)
{
	die('ERROR!!!.Data already Update.');
}

if(strlen($dpd_meeting_sd_mm_dpd_sample_date_post)>0)
{
$SQL="UPDATE `tb_track_info` SET `dpd_meeting_sd_mm_dpd_sample_date` = '$dpd_meeting_sd_mm_dpd_sample_date_post' WHERE `tb_track_info`.`sl` ='$sl' AND brand_style='$update_brand_style'" ;
$obj->sql($SQL);	
