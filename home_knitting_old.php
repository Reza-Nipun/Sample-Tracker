...
//	$SQL="select *,a.sl as sll,a.color_code as c_code from tb_fabric_booking a,tb_track_info b WHERE b.sd_concern_mm_name='$user_name' AND a.sample_style=b.style AND b.color=a.color";    //table name
	...
	$sql= $sql . "AND STR_TO_DATE( $select_logic,  '%d-%m-%Y' ) between STR_TO_DATE( '$date',  '%d-%m-%Y' ) and STR_TO_DATE( '$date1',  '%d-%m-%Y')" ; 
	 ...
	 $sql= $sql . " AND a.knit_status NOT IN ('Complete', 'Cancelled') AND a.sto_po_no!='' group by a.sl order by sll, length(sll) ASC " ;
...
$result = $obj->sql($sql);
	...
	while($row = mysql_fetch_array($result))
	{
		...
		<tr id="<?php echo $row['sll'] ; ?>" class="edit_tr">
		  ...
		  <!--<td><input name="checkbox[]" type="checkbox" id="checkbox[]" value="<?php // echo $row['sll']; ?>"></td>-->
			  ...
			  <td<?php echo $bgc; ?> title="<?php echo $row['sd_concern_mm_name'] ; ?>"><?php  if($cancel_status != 0) { ?><a href="JavaScript:newPopup('archive_knitting.php?fab_sl=<?php echo $row['sl'] ; ?>');" title="Click to Remove from Work Space" style="color:#036"> <img src="images/Flag-red-icon.png"></a><?php } ?><?php echo $si ; ?></td>
    ...
    <td<?php if($revise_status != 0) { ?> bgcolor="#88E4EC" <?php } if($cancel_status != 0) { ?> style="color:#F00"<?php } ?>><?php echo $row['gsm'] ;  ?></td>
...
<td<?php if($revise_status != 0) { ?> bgcolor="#88E4EC" <?php } if($cancel_status != 0) { ?> style="color:#F00"<?php } ?>><?php echo $row['cuff_qty'] ;  ?></td>
    ...
    <td<?php if($revise_status != 0) { ?> bgcolor="#88E4EC" <?php } if($cancel_status != 0) { ?> style="color:#F00"<?php } ?>><?php echo $row['fab_wash'] ;  ?></td>
    ...
    <td bgcolor="#CCCCFF"><?php echo $row['referrence_id']?></td>
     <td<?php if($revise_status != 0) { ?> bgcolor="#88E4EC" <?php } if($cancel_status != 0) { ?> style="color:#F00"<?php } ?>><?php echo $row['sto_po_no']; ?></td>
