...
<table cellpadding="0" align="center" cellspacing="0" border="1" class="bottomBorder draggable sortable" id="example" style="width:97% !important; padding-bottom: 10px; background-color: #FFF; color:#000;">
 <!-- <thead align="center" style="font-weight:bold; font-size:11px; color:#000" bgcolor="#A0F21B">-->
  ...
  <tr bgcolor="#FFCC33">
    ...
    <th>DPD Fabric<br/>Booking Qty</th>
    <th>Knit Fabric<br/>Delv Qty</th>
    <th>Knit Delv<br/>Blnc. Qty</th>
    <th>STO Status</th>
...
/*if($customer!=NULL OR $status!=NULL OR $date1!=NULL OR $date2!=NULL)
{
...
{*/
	...
	$sql = "SELECT A.*, IFNULL(B.DPD_REQ_QTY, 0) AS 'DPD_REQ_QTY', IFNULL(B.KNIT_DELV_QTY, 0) AS 'KNIT_DELV_QTY', IFNULL(B.KNIT_BALANCE_QTY, 0) AS 'KNIT_BALANCE_QTY', B.sto_po_no  FROM (
SELECT create_date, sd_sample_req_rcv_date, customer, season, style, color, sd_sample_type_name, SUM( qty ) AS 'SD_Qty', mm_fabricbooking_by_dpd_to_mm_date, sd_agreed_sample_delivery_date, sd_actual_sample_submission_date, sd_concern_sd_name, sd_concern_dpd_name, sd_concern_mm_name, sd_sample_reject_pass FROM `tb_track_info` 
WHERE STR_TO_DATE( create_date, '%d-%m-%Y' ) 
BETWEEN STR_TO_DATE( '$date1', '%d-%m-%Y' ) 
AND STR_TO_DATE( '$date2', '%d-%m-%Y' ) 
GROUP BY style, color, season

) A LEFT JOIN (

SELECT sample_style, T0.color, T0.season, SUM( dpd_req_qty ) AS 'DPD_req_qty', SUM( knit_del_qty ) AS 'Knit_delv_qty', (
SUM( dpd_req_qty ) - SUM( knit_del_qty )
) AS 'knit_balance_qty', sto_po_no 
FROM `tb_fabric_booking` T0
LEFT JOIN tb_track_info T1 ON T1.sl = T0.track_info_sl
WHERE STR_TO_DATE( create_date, '%d-%m-%Y' ) 
BETWEEN STR_TO_DATE( '$date1', '%d-%m-%Y' ) 
AND STR_TO_DATE( '$date2', '%d-%m-%Y' ) 
GROUP BY `sample_style` , T0.color, T0.season

) B ON B.sample_style = A.style AND B.color = A.color AND B.season = A.season";




	if($customer!=NULL)
	 { $sql= $sql . " AND A.customer='$customer'" ; }
	 ...
	 if($status!=NULL)
	 {
		if($status=='Due')
		{
		$sql= $sql . " AND A.sd_sample_reject_pass=''" ;	
		}
		else
		{
		$sql= $sql . " AND A.sd_sample_reject_pass='$status'" ; 
		}
	 }
...
<td align="left"><a href="style_details.php?IDX=<?php echo $row['style'] ; ?>" style="color:#000" title="View Style Details" target="_blank"><?php echo $row['style'] ; ?></a></td>
    ...
    <td align="center"><?php echo $row['SD_Qty'].' PC' ; ?></td>
    <?php 
...
*/	
  ...
  <td align="center"><?php echo $row['DPD_req_qty'] ; ?></td>
  <td align="center"><?php echo $row['Knit_delv_qty'] ; ?></td>
  <td align="center"><?php echo $row['knit_balance_qty'] ; ?></td>
