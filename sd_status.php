...
<html>
	...
	<body>
		...
		<div id="demo" style="margin-top:80px; ">
			<form action="sd_status_update.php" method="post">
				<table cellpadding="0" cellspacing="0" border="1" class="draggable sorttable" id="example" style="width:100% !important; padding-bottom: 10px; background-color: #FFF; color:#000;">
					...
					<thead>
						<tr bgcolor="" style="color:#000; font-family:'Courier New', Courier, monospace; font-weight:normal">
							...
							<th style="">Concern Dpd</th>
							<th style="">Actual Submission (Date)</th>
							<th style="">Remark SD</th>
					...
					<tbody>
						<?php
							...
							else
							{
								...
								//ajhid
					/*			SELECT a.sd_concern_sd_name, COUNT( a.sd_sample_reject_pass ) AS aa, a.sd_agreed_sample_delivery_date
FROM tb_track_info a
LEFT JOIN tb_admin b ON b.rule =  '1'
AND b.status =  '1'
AND a.sd_concern_sd_name = b.user_name
WHERE a.sd_sample_reject_pass =  ''
AND STR_TO_DATE( a.sd_agreed_sample_delivery_date,  '%d-%m-%Y' ) 
BETWEEN STR_TO_DATE(  '01-09-2013',  '%d-%m-%Y' ) 
AND STR_TO_DATE(  '14-07-2014',  '%d-%m-%Y' ) 
GROUP BY a.sd_concern_sd_name

*/
																
							$sql="select * from tb_track_info WHERE sd_sample_reject_pass='' AND  sd_concern_sd_name='$user_name' AND STR_TO_DATE( sd_agreed_sample_delivery_date,  '%d-%m-%Y' ) between STR_TO_DATE( '01-09-2013',  '%d-%m-%Y' ) and STR_TO_DATE( '$sys_date',  '%d-%m-%Y') order by sl DESC";
