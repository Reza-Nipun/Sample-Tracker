...
<div class="span12">
  ...
  <tr align="center" style="font-weight:bold; font-size:11px">
    ...
    <td width="8%">Style</td>
    <td width="12%">
      <input name="S_ID" class="rounded" list="characters2" type="text" autofocus required id="S_ID" value="" size="12">
            
              <datalist id="characters2">
              				<?php
         					$SQL="select style from tb_track_info WHERE sd_concern_dpd_name='$user_name' AND sd_actual_sample_submission_date='' group by style order by style ASC";
                            $obj->sql($SQL);
    ...
    <td width="10%"><select name="season" autofocus id="season" style="font-size:11px">
      <option value="">--Select season--</option>
      <?php 
		  $SQL="select season from tb_track_info WHERE sd_concern_dpd_name='$user_name' AND sd_actual_sample_submission_date='' group by season"; 
         		$result=$obj->sql($SQL);
...
//	$results = $obj->sql($SQL);
	...
	if($user_rule=='6')
	{
			$sql="select a.*,a.sl as sll,b.sd_concern_dpd_name,b.brand_style,b.fab_receive_planned_date,b.sd_agreed_sample_delivery_date,b.sd_sample_type_name,b.customer,a.color_code as c_code from tb_fabric_booking a,tb_track_info b WHERE b.style=a.sample_style AND b.color=a.color ";    //table name
	}	
