...
<span>Samples Tracker (GDL) </span></h1>
  ...
  <tr>
    <th colspan="5" style="color:#000" scope="col"><h2>GDL Sample Print From <?php echo $date1 ; ?> to <?php echo $date2 ; ?> </h2></th>
    </tr>
  <tr>
...
<table id="gradient-style" align="left" class="draggable sortable" width="100%" border="1" cellspacing="0" cellpadding="0">
  ...
  <tr>
	...
	// $SQL3a = "SELECT T0.customer, T0.ttl , COALESCE(T1.cnt, 0) AS 'cnt', COALESCE((T0.ttl-T1.cnt), 0) AS 'Pending' FROM (SELECT  customer , COUNT( style ) AS ttl FROM  `tb_track_info` GROUP BY  customer) T0 LEFT JOIN (SELECT  customer ,  COUNT(style) AS cnt FROM  tb_track_info WHERE  mm_fabricbooking_by_dpd_to_mm_date !=  '' GROUP BY customer) T1 on T1.customer = T0.customer";
	 ...
	 $SQL3 = "SELECT customer , SUM(gdl_cut_panel) AS tcp, SUM(gdl_cut_panel_dlv) as tcpd, gdl_date FROM  `tb_track_info` WHERE STR_TO_DATE( gdl_date,  '%d-%m-%Y' ) between STR_TO_DATE( '$date1',  '%d-%m-%Y' ) and STR_TO_DATE( '$date2',  '%d-%m-%Y' ) GROUP BY customer ORDER BY customer ASC "; // The Avobe $SQL3a Query is also Correct
...
<table id="gradient-style" align="left" class="draggable sortable" width="100%" border="1" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <th width="5%" style="font-size:16px; font-weight:bold; color:#000; font-color:#000">sl</th>
    <th width="35%" style="font-size:16px; font-weight:bold; color:#000; font-color:#000"><strong>Print Type</strong></th>
    <th width="20%"style="font-size:16px; font-weight:bold; color:#000; font-color:#000">Tiotal Cut Panel Rcv</th>
    <th width="20%"style="font-size:16px; font-weight:bold; color:#000; font-color:#000">Total Cut Panel Dlv</th>
    </tr>
    </thead>                        
	 ...
	 $t=0 ;
	 $sl1=1;
	$SQL="SELECT gdl_print_type , SUM(gdl_cut_panel) AS tcpx, SUM(gdl_cut_panel_dlv) as tcpdx, gdl_date FROM  `tb_track_info` WHERE STR_TO_DATE( gdl_date,  '%d-%m-%Y' ) between STR_TO_DATE( '$date1',  '%d-%m-%Y' ) and STR_TO_DATE( '$date2',  '%d-%m-%Y' ) GROUP BY gdl_print_type ORDER BY gdl_print_type ASC"; 
$results = $obj->sql($SQL);
	...
	<tr style="color:#000">
    <td align="center"><?php echo $sl1 ; ?></td>
    <td align="center"><?php echo $row['gdl_print_type'] ; ?></td>
...
<section class="span12">
<h2>PRINT Color TYPE WISE SAMPLE SUMMERY VIEW</h2>

<table id="gradient-style" align="left" class="draggable sortable" width="100%" border="1" cellspacing="0" cellpadding="0">
  <thead align="center" style="font-weight:bold; font-size:11px; color:#000" bgcolor="#A0F21B">
  <tr>
    <th width="5%" style="font-size:16px; font-weight:bold; color:#000; font-color:#000">sl</th>
    <th width="25%" style="font-size:16px; font-weight:bold; color:#000; font-color:#000"><strong>Print Type</strong></th>
    <th width="25%" style="font-size:16px; font-weight:bold; color:#000; font-color:#000"><strong>Print Color</strong></th>
    <th width="15%"style="font-size:16px; font-weight:bold; color:#000; font-color:#000">Tiotal Cut Panel Rcv</th>
    <th width="15%"style="font-size:16px; font-weight:bold; color:#000; font-color:#000">Total Cut Panel Dlv</th>
    </tr>
    </thead>                        
	 ...
	 $t=0 ;
	 $sl1=1;
	$SQL="SELECT gdl_print_type ,gdl_print_color, SUM(gdl_cut_panel) AS tcpx, SUM(gdl_cut_panel_dlv) as tcpdx, gdl_date FROM  `tb_track_info` WHERE STR_TO_DATE( gdl_date,  '%d-%m-%Y' ) between STR_TO_DATE( '$date1',  '%d-%m-%Y' ) and STR_TO_DATE( '$date2',  '%d-%m-%Y' ) GROUP BY gdl_print_type,gdl_print_color ORDER BY gdl_print_type,gdl_print_color ASC"; 
$results = $obj->sql($SQL);
	while($row = mysql_fetch_array($results))
	{       
    ...
    <td align="center"><?php echo $sl1 ; ?></td>
    <td align="center"><?php echo $row['gdl_print_type'] ; ?></td>
    <td align="center"><?php echo $row['gdl_print_color'] ; ?></td>
    <td align="center"><?php echo $row['tcpx'] ; ?></td>
    <td align="center"><?php echo $row['tcpdx'] ; ?></td>
    </tr>
	
                      	  <?php
	$tcp1xy=$tcp1xy+$row['tcpx'] ;
	$tcpd2xy=$tcpd2xy+$row['tcpdx'] ;
	...
	?>
	 ...
	 <td align="center"><strong>Total</strong></td>
     <td align="center"></td>
	 <td align="center"><strong><?php echo $tcp1xy ; ?></strong></td>
     <td align="center"><strong><?php echo $tcpd2xy ; ?></strong></td>
	 </tr>
