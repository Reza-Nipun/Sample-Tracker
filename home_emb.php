...
if(first != '' || midle != '' || comments != '' || status != '' || status_print != '')
{
$.ajax({
type: "POST",
url: "home_emb_update.php",
data: dataString,
cache: false,
...
<body>
   ...
   <div class="span12">
                                <h2>SAMPLE TRACKER Embroidery Part</h2>
                                <div class="divider"></div>
<div width="80%"  style="margin-top:10px; ">


  <form action="home_emb.php" method="POST">
...
<?php if($search== 1) { ?>
<form action="home_emb_workspace_update.php" method="post">    
...
<table cellpadding="0" cellspacing="0" border="1" class="draggable" id="example" style="width:1800px !important; padding-bottom: 10px; background-color: #FFF; color:#000;">
    ...
    <th bgcolor="#CCCCFF">Emb. Cut Panel Rcv</th>
    <th bgcolor="#CCCCFF">Emb. Cut Panel Dlv</th>
    <th style="width:120px" bgcolor="#CCCCFF">Emb. Comt</th>
    <th style="width:80px" bgcolor="#CCCCFF">Emb. Status</th>
    <th style="width:80px" bgcolor="#CCCCFF">Embroidery Type</th>
    <th>SD Comt</th>
    <th>DPD Comt</th>
    <th>MM Comt</th>
    <th>Sample</th>
    <th><strong>Embroidery Comt</strong></th>
...
FROM `tb_track_info` WHERE sl!=''";    //table name
	...
	$sql= $sql . " AND STR_TO_DATE( $select_logic,  '%d-%m-%Y' ) between STR_TO_DATE( '$date',  '%d-%m-%Y' ) and STR_TO_DATE( '$date1',  '%d-%m-%Y')" ; 
	 ...
	 $sql=$sql." AND sd_actual_sample_submission_date=''  Limit 0 , 3000";
...
<span onfocus="this.select()" id="first_<?php echo $row['sl']; ?>" class="text"><?php echo $row['emb_cut_panel']?></span>
      <input type="number" class="editbox" onClick="this.select()" onfocus="this.select()" id="first_input_<?php echo $row['sl']; ?>" value="<?php echo $row['emb_cut_panel'] ; ?>" style="width:50px" size="4" maxlength="4" /></td>
      
     
      <strong> <td class="edit_td" bgcolor="#66FFFF">
      <span onfocus="this.select()" id="midle_<?php echo $row['sl']; ?>" class="text"><?php echo $row['emb_cut_panel_dlv']?></span>
      <input type="number" class="editbox" onClick="this.select()" onfocus="this.select()" id="midle_input_<?php echo $row['sl']; ?>" value="<?php echo $row['emb_cut_panel_dlv'] ; ?>" style="width:50px"  size="4" maxlength="4" /></td></strong>
...
<a href="JavaScript:newPopup1('home_gdl_cmt_update.php?SID=<?php // echo $row['sl'] ; ?>');" style="color:#030"><strong>Add+</strong></a><?php // }  ?></td>-->
	  ...
	  <strong> <td class="edit_td" bgcolor="#66FFFF">
      <span onfocus="this.select()" id="comments_<?php echo $row['sl']; ?>" class="text"><?php echo $row['emb_comnt']?></span>
      <textarea class="editbox" onClick="this.select()" onfocus="this.select()" id="comments_input_<?php echo $row['sl']; ?>" rows="3" cols="30" ><?php echo $row['emb_comnt'] ; ?></textarea></td></strong>
      
      
<td class="edit_td" bgcolor="#66FFFF">
<span onfocus="this.select()" id="status_<?php echo $row['sl']; ?>" class="text"><?php echo $row['emb_status']?></span>
<select class="editbox" onClick="this.select()" onfocus="this.select()" id="status_input_<?php echo $row['sl']; ?>">
<option>Select Status</option>
<option value="<?php echo $row['emb_status'] ; ?>" selected="selected"><?php echo $row['emb_status'] ; ?></option>
...
<td class="edit_td" bgcolor="#66FFFF">
<span onfocus="this.select()" id="status_print_<?php echo $row['sl']; ?>" class="text"><?php echo $row['emb_emb_type']?></span>
<select class="editbox" onClick="this.select()" onfocus="this.select()" id="status_print_input_<?php echo $row['sl']; ?>">
<option value="">Select Type</option>



 <option value="<?php echo $row['emb_emb_type'] ; ?>" selected><?php echo $row['emb_emb_type'] ; ?></option>
			  <?php
         $SQL8="select concern_name from tb_concern where concern_type like 'emb_type' order by concern_name ASC";
                            $obj->sql($SQL8);
