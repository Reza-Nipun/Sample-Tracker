...
$SQLX="select create_date,customer,brand_style,style, qty,sd_agreed_sample_delivery_date,season,fabric_type,print_type,wash_type,product_type,embroidery_stitch, sd_concern_dpd_name from tb_track_info where style='$h_style' AND season='$h_season'";    //table name
	...
	while($rowx = mysql_fetch_array($resultsx))
	{
		...
		$sd_concern_sd_name=$rowx['sd_concern_sd_name'];
...
<table align="center" width="80%" border="1" cellspacing="0" class="bottomBorder" cellpadding="0">
  ...
  <tr align="center">
    ...
    <td><strong>Concern SD</strong></td>
  ...
  <tr align="center">
    ...
    <td><strong>Embroidery Stitch</strong></td>
...
<tr>
 ...
 <?php 
	...
	$sql_fetch="SELECT T0.*, T1.fab_receive_planned_date, T1.sd_sample_type_name from tb_fabric_booking T0 LEFT JOIN tb_track_info T1 ON T1.sl = T0.track_info_sl WHERE T0.sl='$checkbox'";    //table name
...
?> 
  ...
  <td><?php echo $sl; ?></td>
    <td scope="row"><select name="s_color[]" autofocus required id="s_color[]" tabindex="1" style="font-size:11px">
     <option value="<?php echo $row_fetch['color'].'~'.$row_fetch['track_info_sl']; ?>" selected="selected"><?php echo $row_fetch['color'].' - '.$row_fetch['sd_sample_type_name']; ?></option>
        <?php
		
		
         $SQL="select sl, color, sd_sample_type_name from tb_track_info where style like '$style' AND season='$season' ORDER BY sl DESC";
		 //die($SQL);
		  $obj->sql($SQL);
