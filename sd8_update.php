...
$sd_sample_reject_pass_post=mysql_real_escape_string($_POST['sd_sample_reject_pass']);

if($sd_sample_reject_pass!=NULL)
{
	die('ERROR!!!.Data already Update.');
}

if(strlen($sd_sample_reject_pass_post)>0)
{
	
$SQL="UPDATE `tb_track_info` SET `sd_sample_reject_pass` = '$sd_sample_reject_pass_post' WHERE `tb_track_info`.`sl` ='$sl' AND brand_style='$update_brand_style'" ;
$obj->sql($SQL);	
