...
$mm_trims_actual_date_post=mysql_real_escape_string($_POST['mm_trims_actual_date']);

if($mm_trims_actual_date!=NULL)
{
	die('ERROR!!!.Data already Update.');
}

if(strlen($mm_trims_actual_date_post)>0)
{
	
$SQL="UPDATE `tb_track_info` SET `mm_trims_actual_date` = '$mm_trims_actual_date_post' WHERE `tb_track_info`.`sl` ='$sl' AND brand_style='$update_brand_style'" ;
$obj->sql($SQL);	
